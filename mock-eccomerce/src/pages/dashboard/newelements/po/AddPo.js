...
export default function AddPo() {
  ...
  const ordersLoc = collection(db, 'po')
  const [customers, setCustomers] = useState(null)
  const custsLoc = collection(db, 'vendors')
  ...
  async function handleSubmit(e) {
    ...
    const docRef = await addDoc(collection(db, 'po'), orderData)
    history(`/dashboard/po/${docRef.id}`)
  }
            ...
            <div className='customer-subdetails'>
              <div className="fields">
                ...
                <select required onChange={() => handleSelect(customerRef.current.value)} className='vendor-selector' ref={customerRef} defaultValue=''>
                  <option disabled hidden value='' >Select a Vendor</option>
