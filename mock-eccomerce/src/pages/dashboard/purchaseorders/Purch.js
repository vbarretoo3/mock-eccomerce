...
export default function Purch() {
  ...
  const docSnap = async() => {
    ...
    const custArr = []
    const custLoc = collection(db, 'vendors')
  ...
  function handleCustomerChange() {
    for (let x of customers) {
      if (x.id === nameRef.current.value) {
        order.vendor.name = x.data.name
        order.vendor.vendorRef = x.id
        order.vendor.address = x.data.address
        order.vendor.contact = x.data.contact
      }
    }
    setUpdate((currState) => currState + 1)
  }
      ...
      <>
        <div className='customer-subdetails'>
          <div className="fields">
            ...
            <select className='vendor-selector' onChange={handleCustomerChange} defaultValue={order.vendor.vendorRef} ref={nameRef} required>
        ...
        <div className='customer-subdetails'>
          <div className="fields">
            ...
            <p>{order.vendor.address.address}</p>
            <h3>
              Province:
            </h3>
            <p>{order.vendor.address.province}</p>
          </div>
          <div className="fields">
            <h3>
              City:
            </h3>
            <p>{order.vendor.address.city}</p>
            <h3>
              Postal Code:
            </h3>
            <p>{order.vendor.address.postal}</p>
          </div>
        </div>
      </>
    )
